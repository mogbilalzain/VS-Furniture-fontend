...
export default function MaterialsAdmin() {
  ...
  return (
    <div className="admin-legacy space-y-6">
      <PageHeader
        title="Materials"
        description="Manage individual materials with colors and images (M030, F010, L027, etc.)"
        actions={
          <AdminButton variant="primary" size="lg" icon="add" onClick={handleAdd} disabled={groups.length === 0}>
            Add New Material
          </AdminButton>
        }
      />
      ...
      {groups.length === 0 && (
        <div className="px-5 py-3 rounded-2xl bg-primary-fixed/40 border border-primary-fixed-dim/50 text-on-primary-fixed-variant flex items-center gap-3">
          ...
          Please create at least one material group before adding materials.
        </div>
      )}

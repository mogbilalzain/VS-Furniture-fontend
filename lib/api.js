...
/**
 * يعيد كتابة أي رابط صورة قديم بصيغة /storage/images/... إلى /uploads/images/...
 * (الخادم بعد التوحيد يخدم الصور تحت /uploads/ فقط).
 *
 * يعمل على:
 *   - النص الكامل: "https://vsme.ae/storage/images/..." -> "https://vsme.ae/uploads/images/..."
 *   - النص النسبي: "/storage/images/..."                 -> "/uploads/images/..."
 *
 * يتجاهل أي قيمة ليست نصاً.
 */
function rewriteLegacyImageString(value) {
  if (typeof value !== 'string' || value.length === 0) return value;
  return value.replace(/\/storage\/(images\/)/g, '/uploads/$1');
}

/**
 * يجتاز الكائن (data) ويعيد كتابة كل القيم النصية التي تبدأ بـ /storage/images/...
 * بشكل غير مدمر (deep clone فقط للأجزاء التي لمسناها).
 */
function rewriteLegacyImageUrls(data) {
  if (data === null || data === undefined) return data;
  if (typeof data === 'string') return rewriteLegacyImageString(data);
  if (Array.isArray(data)) return data.map(rewriteLegacyImageUrls);
  if (typeof data === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(data)) {
      out[k] = rewriteLegacyImageUrls(v);
    }
    return out;
  }
  return data;
...
class ApiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    const apiPathWithQuery = url.replace(this.baseURL, '');
    const apiPath = apiPathWithQuery.split('?')[0];
    ...
    const isPublicProductCategory = (
      ...
      apiPath.match(/^\/categories\/\d+\/properties\/faceted$/) ||
      apiPath.match(/^\/categories\/\d+\/property-groups$/) ||
      // Materials endpoints
    ...
    try {
      ...
      // طبقة دفاع: إذا أرجع الخادم أي روابط /storage/images/... قديمة (بسبب كاش
      // أو نشر متأخر)، نُعيد كتابتها فوراً إلى /uploads/images/... كي يعمل
      // عرض الصور على Next.js دون انتظار redeploy.
      const normalized = rewriteLegacyImageUrls(data);
...
export const productsAPI = {
  ...
  update: (id, productData) => {
    if (productData instanceof FormData) {
      ...
      return apiClient.postFormData(`/admin/products/${id}`, productData);
    }
...
export const propertiesAPI = {
  ...
  getFacetedProperties: (categoryId, categorySlug, filters = {}) => {
    const queryParams = new URLSearchParams();
    if (categorySlug) queryParams.append('category_slug', categorySlug);
    Object.entries(filters).forEach(([propertyName, values]) => {
      if (Array.isArray(values) && values.length > 0) {
        values.forEach(value => {
          queryParams.append(`filters[${propertyName}][]`, value);
        });
      }
    });
    const qs = queryParams.toString();
    const url = `/categories/${categoryId}/properties/faceted${qs ? '?' + qs : ''}`;
  ...
  // Property Groups
  getAllPropertyGroups: () => apiClient.get('/admin/property-groups'),
  getPropertyGroups: (categoryId) => apiClient.get(`/categories/${categoryId}/property-groups`),
  createPropertyGroup: (categoryId, data) => apiClient.post(`/admin/categories/${categoryId}/property-groups`, data),
  updatePropertyGroup: (groupId, data) => apiClient.put(`/admin/property-groups/${groupId}`, data),
  deletePropertyGroup: (groupId) => apiClient.delete(`/admin/property-groups/${groupId}`),
};
...
// Site-wide settings (maintenance flag, future system toggles)
export const settingsAPI = {
  // Public: any visitor can read this to know whether to render the maintenance page
  getMaintenance: () => apiClient.get('/settings/maintenance'),
  admin: {
    getAll: () => apiClient.get('/admin/settings'),
    setMaintenance: (enabled) => apiClient.put('/admin/settings/maintenance', { enabled: !!enabled }),
  }
};
